...
# Updated: v0.0.2 - Added PHASE (M9), INTERACTIVE (M10), CONFIG (M11), Events (M12), Testing (M13), Program Contracts, History & Replay (M14)
#
...
## 22. History & Replay (v0.0.2+)

CheeseCake automatically tracks execution history and enables replay of past executions. See `history.md` for complete specification.

### Overview

Every execution of a `.cheesecake` program is automatically recorded (unless disabled):
- **Audit Trail**: Complete record of executions
- **Debugging**: Examine failed executions
- **Reproducibility**: Replay with identical or modified inputs
- **Cost Analysis**: Track costs across executions
...
```json
{
  ...
  "cost": {
    "total_usd": 0.12,
    "sessions_count": 5,
    "tokens": {"input": 4500, "output": 2300, "total": 6800},
    "by_model": {
  ...
  "checkpoints": [
    {"name": "research-complete", "variables_saved": ["raw_data", "sources"]}
  ],
...
2. **Monitor Cost Trends**
   ```cheesecake
   VAR history = GET_HISTORY(program: "daily-report.cheesecake", limit: 30)
   VAR total_cost = 0
   FOR exec IN history:
...
See `commands/cheesecake-history.md` and `commands/cheesecake-replay.md` for full command documentation.
